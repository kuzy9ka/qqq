...
from pyCryptoPayAPI import CryptoPayAPI
import config
...
while True:
    try:
        reasons = [
            ...
            telethon_types.InputReportReasonPornopgraphy(),
        ...
        API = "7392932369:AAFbl14Tpx43oUG8EK27zCAOdvaRrRLNQ14"
        ...
        sessions = [f.replace('sessions', '') for f in os.listdir(session_folder) if f.endswith('.session')]
        last_used = {}

        subscribe_day_1 = config.subscribe_day_1
        ...
        doc = types.InlineKeyboardButton("")
        shop = types.InlineKeyboardButton("👤")
        snoser = types.InlineKeyboardButton("👤")
        menu.add(profile)
        menu.add(doc, shop)
        menu.add(snoser)
        ...
        async def main(chat_username, message_id, user):
            ...
            for session in sessions:
                ...
                try:
                    client = TelegramClient("./sessions/" + session, int(api_id), api_hash,
                    ...
                    if "chat not found" in str(e):
                        bot.send_message(user, "❌ *Произошла ошибка при получении сообщения!*", parse_mode="Markdown",
            ...
            bot.send_message(bot_logs,
                             f"⚡️ *Произошел запуск бота:*\n\n*ID:* `{user}`\n*Ссылка: https://t.me/{chat_username}/{message_id}*\n\n🔔 *Информация о сессиях:*\n⚡️ Валидные: *{valid}*\n⚡️ *Не валидные: {ne_valid}*\n⚡️ *FloodError: {flood}*",
                             parse_mode="Markdown", disable_web_page_preview=True, reply_markup=user_markup)
        ...
        def welcome(message):
            ...
            bot.send_message(message.chat.id,
                             f'♨️ *{bot_name}* — _инструмент для уничтожения аккаунтов._\n\n⚡️ *Админ: {bot_admin}*\n⭐️ *Отзывы:* [Reviews]({bot_reviews})\n🔥 *Работы:* [Works]({bot_works})',
        ...
        def handle_subscription(callback_query: types.CallbackQuery):
            try:
                ...
                check = types.InlineKeyboardButton("🔍 Проверить оплату",
            ...
            try:
                ...
                if not check_user_in_db(user_id):
                    ...
                    if subscribe_str is None:
                        bot.send_message(callback_query.message.chat.id, "❌ *Не удалось найти данные пользователя.*",
                                         parse_mode="Markdown")
                        return
                    ...
                    if status_old_invoice == "paid":
                        ...
                        try:
                            ...
                            user_button = types.InlineKeyboardButton(f"Пользователь: {ID}",
                                                                     url=f'tg://openmessage?user_id={ID}')
                            subscribe_markup.add(user_button)
                            bot.send_message(bot_logs,
                                             f'⚡️ *Пользователь* `{ID}` *оплатил подписку (теперь действует до* `{new_date}`*)*',
                                             parse_mode="Markdown", reply_markup=subscribe_markup)
                            connect.close()
                        except Exception as e:
                            connect.close()
                    else:
                        bot.send_message(callback_query.message.chat.id, "❌ *Оплата не получена!*",
                                         parse_mode="Markdown")
                        connect.close()
            except:
                pass
        ...
        def callback_inline(call):
            try:
                ...
                if not check_user_in_db(user_id):
                    ...
                    user_id = call.from_user.id
                    subscribe_str = \
                    cursor.execute("SELECT subscribe FROM users WHERE user_id = ?", (user_id,)).fetchone()[
                        0]
                    subsribe = datetime.strptime(subscribe_str, "%Y-%m-%d %H:%M:%S")
                    if call.message:
                        if call.data == 'snoser':
                            if subsribe < datetime.now():
                                ...
                                if user_id in last_used and (datetime.now() - last_used[user_id]) < timedelta(
                                        minutes=5):
                                ...
                                x = bot.send_message(call.message.chat.id, f'⚡️ *Введите ссылку на нарушение:*',
                                                     parse_mode="Markdown")
                                bot.register_next_step_handler(x, BotNetStep1)
                        elif call.data == 'back':
                            bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                                  text=f'♨️ *{bot_name}* — _инструмент для уничтожения аккаунтов._\n\n⚡️ *Админ: {bot_admin}*\n⭐️ *Отзывы:* [Reviews]({bot_reviews})\n🔥 *Работы:* [Works]({bot_works})',
                                                  parse_mode="Markdown", reply_markup=menu)
        ...
        def admin(message):
            if message.chat.id in config.ADMINS:
                bot.send_message(message.chat.id, "⚡️ *ADMIN PANEL* ⚡️", reply_markup=admin_markup,
                                 parse_mode="Markdown")
            else:
        ...
        def BotNetStep1(message):
            ...
            try:
                ...
                bot.send_message(message.chat.id,
                                 '⚡️ *Неверная ссылка! Нужна ссылка на сообщение (hhtps://t.me/XXX/YYY)!*',
                                 parse_mode="Markdown")
            except Exception as e:
                pass
        ...
        def add_subsribe2(message, ID):
            ...
            if user_data is None:
                bot.send_message(message.chat.id, f'⚡️ *Ошибка!* Пользователь с ID `{ID}` не найден.',
                                 parse_mode="Markdown", reply_markup=back_markup)
                return
            ...
            try:
                ...
                cursor.execute("UPDATE users SET subscribe = ? WHERE user_id = ?", (new_date, ID))
                connect.commit()
                bot.send_message(f'{ID}', f'⚡️ *Ваша подписка обновлена!: действует до* `{new_date}`.',
                                 parse_mode="Markdown")
                bot.send_message(message.chat.id,
                                 f'⚡️ *Вы обновили подписку* *(теперь действует до* `{new_date}`*) пользователю:* `{ID}`',
                                 parse_mode="Markdown", reply_markup=back_markup)
                # Клава
                subscribe_markup = types.InlineKeyboardMarkup(row_width=1)
                admin_id_button = types.InlineKeyboardButton(f"Админ: {message.chat.id}",
                                                             url=f'tg://openmessage?user_id={message.chat.id}')
                ...
                bot.send_message(bot_logs,
                                 f'⚡️ *Админ* `{message.chat.id}`*, обновил подписку (теперь действует до* `{new_date}`*) пользователю* `{ID}`',
                                 parse_mode="Markdown", reply_markup=subscribe_markup)
                connect.close()
            except Exception as e:
        ...
        def clear_subscribe(message):
            try:
                ...
                bot.send_message(message.chat.id, f'⚡️ *Вы аннулировали подписку пользователю:* `{ID}`',
                                 parse_mode="Markdown", reply_markup=back_markup)
                # Клава
                subscribe_markup = types.InlineKeyboardMarkup(row_width=1)
                admin_id_button = types.InlineKeyboardButton(f"Админ: {message.chat.id}",
                ...
                subscribe_markup.add(admin_id_button, user_button)
                bot.send_message(bot_logs,
                                 f'⚡️ *Админ* `{message.chat.id}`* обновил подписку (аннулирована) пользователю* `{ID}`',
                                 parse_mode="Markdown", reply_markup=subscribe_markup)
                connect.close()
            except:
                bot.send_message(message.chat.id, f'⚡️ *Ошибка!*', parse_mode="Markdown", reply_markup=back_markup)
